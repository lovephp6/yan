...
<div>
    ...
    <div class="page-container">
        <div class="text-c"> 日期范围：
            ...
            <button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜投诉</button>
        </div>
{{--        <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="product_add('添加技师','{{ url('teacher/add') }}')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加技师</a></span> <span class="r">共有数据：<strong>54</strong> 条</span> </div>--}}
        <div class="mt-20">
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                <thead>
                <tr class="text-c">
                    <th width="40">ID</th>
                    <th width="120">城市店面</th>
                    <th width="70">技师姓名</th>
                    <th width="100">上门日期</th>
                    <th width="50">上门时间</th>
		    ...
		    <th width="100">服务时长</th>
                ...
                @foreach($yuyues as $yuyue)
                    <tr class="text-c va-m">
                        <td>{{ $yuyue->id }}</td>
                        <td class="text-l">{{ $yuyue->city }}</td>
                        <td class="text-l">{{ $yuyue->tid }}</td>
                        <td class="text-l">{{ $yuyue->date }}</td>
                        <td class="text-l">{{ $yuyue->time }}</td>
                        <td class="text-l">{{ $yuyue->sid }}</td>
                        <td class="text-l">{{ $yuyue->sid_time }}</td>
                        <td><span class="price">{{ $yuyue->name }}</td>
                        <td>{{ $yuyue->tel }}</td>
                        <td>{{ $yuyue->other }}</td>
                        <td class="td-status">{{ $yuyue->status }}</td>
                        <td class="td-manage"><a style="text-decoration:none" class="ml-5"  href="{{ url('yuyue/edit', $yuyue->id) }}" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="product_del(this,'{{ url('yuyue/delete', $yuyue->id) }}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
